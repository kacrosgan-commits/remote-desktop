# Remote Dragon — Windows quick start
...
- `dist/agent.exe`
- `dist/controller.exe`
- `dist/RemoteDragon-windows.zip` (executables, installer/uninstaller and this guide)
...
2. Double-click the **newly built** `agent.exe` once, as the Windows user
   ...
   `%LOCALAPPDATA%\RemoteDragon` and starts in the background (no success dialog).
...
Uninstall using `%LOCALAPPDATA%\RemoteDragon\uninstall-agent.bat`, as the same
Windows user. Logs are in `%USERPROFILE%\.remote-dragon\agent.log`.
...
1. Open the **newly built** `controller.exe`. The Devices tab shows a live
   ...
   (WebSocket keepalive + 2s device-list sync). Uninstalling an agent on a remote
   PC marks that card Offline — it does **not** close the controller.
